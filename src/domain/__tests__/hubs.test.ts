...
describe('hubs', () => {
  it('attachWireToHub and addHubBridge connect many wires', () => {
    ...
    const [w1, w2] = diagram.conduits[0]!.wireIds;
    ...
    diagram = attachWireToHub(diagram, hubA.id, w1);
    diagram = attachWireToHub(diagram, hubA.id, w2);
  ...
  it('rejects wire link when wire is on a hub', () => {
    ...
    expect(() => addWireLinkToDiagram(diagram, a, b)).toThrow(/hub/i);
  });
  ...
  it('allows only one wire-to-wire link per wire', () => {
    let diagram = createEmptyJob().diagram;
    ...
    diagram = addWireLinkToDiagram(diagram, w1, w2);
    expect(wireLinkForWire(diagram, w1)).toBeDefined();

    expect(() => addWireLinkToDiagram(diagram, w1, w3)).toThrow(/only one/i);
  });
});
